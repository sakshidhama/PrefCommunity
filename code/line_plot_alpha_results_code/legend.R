...
par(mfrow=c(2,5)) # 5X2 graphs
par(oma=c(0,0,5,0))  # top has 5 lines of space 
par(mar=c(4,4,2,1)+.1)  # margin lines
par(pin=c(1.9,1.9,1.9,1.9,1.9))  # plot areas for graphs
...
# add lines
  for (i in 1:19) {
    ...
    y1<-y1+6
    y2<-y2+6
    lines(xrange[x1:x2],yrange[y1:y2], type="b", lwd=0.4,lty=i, col=colors[i+1], pch =plotchar[i+1])
  }
...
legend("right",legend = unlist(line_table[[1]][,2][[1]]), cex=1,ncol=3, col=colors[1:19],lwd = 2.5, pch=plotchar[1:19], lty=1:19, title=paste0("mu values"))
